...
function summarize(items) { return { total:items.length, queued:items.filter(i=>i.status==='queued').length, drafted:items.filter(i=>['draft_generated','ready_for_gmail_draft'].includes(i.status)).length, gmail:items.filter(i=>i.status==='gmail_draft_created').length, manual:items.filter(i=>i.status==='manual_submitted').length, needsKen:items.filter(i=>i.requires_ken || i.status==='needs_ken').length, blocked:items.filter(i=>String(i.safety_status || '').includes('blocked') || i.status==='do_not_contact').length, withEmail:items.filter(i=>getPreferredOutletEmail(i)).length }; }
...
function summarizeDraftMeta(body) {
  ...
  return {
    ...
    hasPlatformSet: ['YouTube:', 'Spotify:', 'Apple Music:', 'Listen / stream:'].some(label => text.includes(label)),
  };
}
